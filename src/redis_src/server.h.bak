...
//这个是redis里的所有的内部结构的编码
//可以看redisObject的结构 unsigned encoding:4; 就是这个值
#define OBJ_ENCODING_RAW 0     /*  原声结构 */
#define OBJ_ENCODING_INT 1     /* int */
#define OBJ_ENCODING_HT 2      /*  hash table */
#define OBJ_ENCODING_ZIPMAP 3  /*  zipmap */
#define OBJ_ENCODING_LINKEDLIST 4 /* 不用了，之前的结构. */
#define OBJ_ENCODING_ZIPLIST 5 /* ziplist */
#define OBJ_ENCODING_INTSET 6  /* Encoded as intset */
#define OBJ_ENCODING_SKIPLIST 7  /* skiplist */
#define OBJ_ENCODING_EMBSTR 8  /* string 低层是sds */
#define OBJ_ENCODING_QUICKLIST 9 /* 底层还是 adlist ziplists */
...
typedef struct zskiplistNode {
    sds ele; // 存贮的数据 为啥不是redisObject
    double score; // 分数
    struct zskiplistNode *backward; //往回走的指针
    struct zskiplistLevel {
        struct zskiplistNode *forward; // 向前的指针，可以向前遍历，找到下一个节点
        unsigned long span; // 表示这个前向指针跳跃过了多少个节点（不包括当前节点)
    } level[]; //结构体zskiplistLevel的数组，表示跳表中的一层。每层又存放有两个字段：
} zskiplistNode;

typedef struct zskiplist {
    struct zskiplistNode *header, *tail; // 头和尾巴
    unsigned long length; // 链表的长度
    int level; // 链表的层级，最大是 32
} zskiplist;
